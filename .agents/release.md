...
`agent-figma` is on the `beta` lane, recorded in `pnpm-workspace.yaml`. Every release is a
prerelease published under the `beta` dist-tag, so nothing reaches `latest` until the lane moves.
...
4. publishes with `pnpm publish --tag beta` over OIDC trusted publishing,
...
```bash
pnpm release:check
pnpm version -r                       # 0.1.0 -> 0.1.0-beta.0, writes the changelog and ledger
git add -A && git commit -m "release 0.1.0-beta.0"
git tag -a v0.1.0-beta.0 -m "release 0.1.0-beta.0"
git push --atomic origin main refs/tags/v0.1.0-beta.0
pnpm build && pnpm publish --tag beta --no-git-checks
```
...
The lane number comes from what is already published: with `0.1.0-beta.4` on the registry, the next
release is `0.1.0-beta.5`. `package.json` carries `0.1.0` until the first release, which is the
...
- **A lane ignores the bump type.** A `minor` intent on a lane produces the next prerelease of the
  ...
  way to `0.1.0`, and it must be re-checked before leaving beta.
- **An unpublished package cannot increment.** Until something is on the registry, every dry run
  says `0.1.0-beta.0`. That is expected before the bootstrap above, and self-corrects after it.
...
That is a visible diff in `pnpm-workspace.yaml`, and it is the only thing standing between a beta
and `latest`. Re-check the bump behavior above first, and drop `publishConfig.tag` and the
`--tag beta` in `release:publish` in the same change.
