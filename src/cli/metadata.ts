...
export const commandMetadata: readonly CommandMetadata[] = [
  ...
  command(["auth", "login"], "Connect a Figma profile with browser OAuth or a personal access token.", AUTHENTICATION, { flags: ["--token", "--profile", "--scopes", "--oauth", "--client-id", "--client-secret", "--redirect-uri", "--no-open", "--auth-url-out", "--timeout-ms", "--json"], output: "sanitized profile status", examples: ["agent-figma auth login", "agent-figma auth login --token \"$FIGMA_TOKEN\" --json"] }),
